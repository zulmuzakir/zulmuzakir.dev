...
export function ProjectCard({
  ...
  const ImageSection = () => (
    <div className="relative mb-6 aspect-video rounded-xl bg-[var(--color-bg-elevated)] overflow-hidden">
      ...
      {/* Hover overlay for compact variant - only show if there's a valid link */}
  ...
  // Links section for full variant - only show if there are links
  const LinksSection = () => {
    const hasGithub = project.github && project.github !== "";
    
    if (!hasValidLink && !hasGithub) {
      return null;
    }
  ...
  const cardClasses = `group block p-8 rounded-2xl bg-[var(--color-bg-card)] border border-[var(--color-border)] hover-lift transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"} ${className}`;
  const cardStyle = { transitionDelay: `${isCompact ? 0.2 + index * 0.1 : 0.1 + (index % 3) * 0.1}s` };

  // Compact variant with valid link - render as anchor
  ...
  // No link or full variant - render as div
  return (
